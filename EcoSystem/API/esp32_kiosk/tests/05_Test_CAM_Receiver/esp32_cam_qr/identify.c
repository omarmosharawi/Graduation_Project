...
static void flood_fill_seed(struct quirc *q, int x, int y, int from, int to,
  ...
  for (;;) {
    ...
    for (;;) {
      ...
      lifo_dequeue(
          &lifo,
          &context); // 这里存疑，如果都没有的话，dequeue就会反向溢出吧。。
      x = context.x;
      y = context.y;
      left = context.l;
      right = context.r;
...
static int region_code(
    struct quirc *q, int x,
    int y) { // region指的是QRcode的区域，成员为区域的坐标，像素面积，是否顶点
  int pixel;
...
static void test_capstone(struct quirc *q, int x, int y, int *pb) {
  ...
  int ring_left =
      region_code(q, x - pb[4] - pb[3] - pb[2] - pb[1] - pb[0], y); // 环左侧
...
static void finder_scan(struct quirc *q, int y) {
  ...
  for (x = 0; x < q->w; x++) {
    ...
    if (x && color != last_color) {           // color is different
      ...
      if (!color &&
          run_count >= 5) { // find the marker of QRcode(three corner's marker)
